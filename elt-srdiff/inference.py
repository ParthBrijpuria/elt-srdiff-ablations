...
def evaluate_model(checkpoint_path, num_batches=20):
    ...
    print(f"--- Starting L={config.max_loops} Inference on {device} ---")
    ...
    print(f"\nEvaluating {num_batches} batches for L={config.max_loops} Ablation...")
    with torch.no_grad():
        for i, batch in enumerate(tqdm(val_loader, total=num_batches)):
            ...
            # Iterative Denoising (loops per timestep defined by config.max_loops)
            ...
            if i == 0:
                ...
                save_image(grid, f"final_L{config.max_loops}_eval_grid.png", nrow=16)

    # 5. Final Results
    print(f"\n=== Final L={config.max_loops} Evaluation Metrics ===")
    print(f"PSNR:  {total_psnr / num_batches:.4f}")
    print(f"SSIM:  {total_ssim / num_batches:.4f}")
    print(f"LPIPS: {total_lpips / num_batches:.4f}")
    print(f"Saved 'final_L{config.max_loops}_eval_grid.png' to the working directory.")
